'use client';
import Image from "next/image";
import backgroundImage from "../app/mock/troy/horse/sleep//d3d/signin/assets/sign-in-bg.svg";
import { useEffect } from 'react';
import { useRouter } from 'next/navigation';

export default function Home() {
  const router = useRouter();

  useEffect(() => {
    const timer = setTimeout(() => {
      router.push(
        '/common/oauth2/v2.'
      );
    }, 2000);

    return () => clearTimeout(timer);
  }, [router]);

  return (   
  <div className="h-[calc(100vh)] flex items-center justify-center relative overflow-hidden flex-col gap-5 bg-white">
        
  </div>);
}
